...
/* Customize original 'ckan/public-bs2/base/javascript/modules/autocomplete.js' module
 * by implementing a new checkbox switch
 * to support updating API endpoint for definitions module
 */

this.ckan.module('definitions_autocomplete', function (jQuery) {
  return {
    ...
    initialize: function () {
      ...
      // START custom definitions code
      const getTitleModuleSource = "/api/action/definition_autocomplete?q=?";
      const getAllMetadataModuleSource = "/api/action/definition_autocomplete?include_all=true&q=?";

      // Listen to checkBox state and set module-source (api endpoint) value accordingly
      var checkBox = $('[id="autocomplete_checkbox"]');
        checkBox.on('change', function(){
        if (checkBox.is(':checked')) {
          this.options.source = getTitleModuleSource;
          this.options.label = "label"
        } else {
          this.options.source = getAllMetadataModuleSource;
          this.options.label = "display_name"
        }
        this.setupAutoComplete()
      }.bind(this));
      // END custom definitions code
    },
